...
export default class GiftedListView extends Component {
        ...
        if (this.props.firstLoader) {
            setTimeout(() => {
                if (this.props.showLoading) {
                    ...
                    this.props.onFetch(this._getPage(), this._postRefresh, { firstLoad: true, showWaiting: false });
                }
                else {
    ...
    _getPaginationStatus() { return this.state.paginationStatus }
    ...
    setNativeProps(props) {
        this.ref.listview.setNativeProps(props);
    }
    ...
    _refreshWithoutWaiting(options = {}) {
    ...
    _onPaginate() {
        if (this.state.paginationStatus === 'allLoaded') {
            ...
            this.props.onFetch(this._getPage() + 1, (rows, options) => this._postPaginate(rows, options), {});
        }
    }
    ...
    render() {
        return (
            <ListView
                ...
                renderSectionHeader={(sectionData, sectionID) => this.props.sectionHeaderView && this.props.sectionHeaderView(sectionData, sectionID)}
    ...
    _renderRow(rowData, sectionID, rowID) {
        let row = parseInt(rowID, 10);
        if ((row % this.props.columns) !== 0) {
            return null;
        }

        let items = [];
        let length = Math.min(this.state.dataSource.getRowCount(), row + this.props.columns);
        for (let i = row; i < length; ++i) {
            let item = this.props.renderRow(this.state.dataSource.getRowData(0, i), sectionID, i);
            items.push(item);
        }
